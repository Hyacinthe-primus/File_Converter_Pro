...
if (-not $IssFile) {
    $IssFile = Join-Path $ProjectRoot "packaging\setup.iss"
} elseif (-not [System.IO.Path]::IsPathRooted($IssFile)) {
    $IssFile = Join-Path $ProjectRoot $IssFile
}

if (-not [System.IO.Path]::IsPathRooted($DistDir))   { $DistDir   = Join-Path $ProjectRoot $DistDir }
