    ...
    def run(self):
        try:
            results = self.db_manager.get_conversion_history(
                limit=self.limit, search_query=self.search_query, start_date=self.start_date, end_date=self.end_date
            )
        except Exception:
            results = []
...
class HistoryDialog(TranslationMixin, QDialog):
    ...
    def setup_ui(self):
        ...
        _unsel = "background-color:transparent;color:#4F46E5;border:2px solid #4F46E5;padding:4px 14px;border-radius:6px;font-weight:bold;"  # noqa: E501
        ...
        self.search_input.setPlaceholderText(
            self.translate_text("Rechercher par nom de fichier ou type d'opération...")
        )
        ...
        self.history_table.setColumnWidth(1, 130)  # Type
        ...
        self.history_table.setColumnWidth(6, 60)  # Status
        hh.setSectionResizeMode(7, QHeaderView.Fixed)
        self.history_table.setColumnWidth(7, 110)  # Actions
    ...
    def open_file(self, row):
        ...
        if os.path.exists(target_file):
            ...
            QMessageBox.warning(
                self, self.translate_text("Fichier introuvable"), self.translate_text("Le fichier cible n'existe plus.")
            )
    ...
    def print_pdf(self):
        ...
        filepath, _ = QFileDialog.getSaveFileName(
            self, self.translate_text("Imprimer en PDF"), "historique_conversions.pdf", "PDF (*.pdf)"
        )
        if not filepath:
            return
        ...
        try:
            ...
            title_style = ParagraphStyle(
                "Title", parent=styles["Title"], fontSize=18, textColor=COLOR_HEADER_BG, spaceAfter=4
            )
            subtitle_style = ParagraphStyle(
                "Sub", parent=styles["Normal"], fontSize=9, textColor=colors.HexColor("#6B7280"), spaceAfter=12
            )
            ...
            story.append(
                Paragraph(
                    self.translate_text("Généré le")
                    + " "
                    + _dt.now().strftime("%d/%m/%Y")
                    + " "
                    + self.translate_text("à")
                    + " "
                    + _dt.now().strftime("%H:%M")
                    + "  —  "
                    + str(self.history_table.rowCount())
                    + " "
                    + self.translate_text("entrée(s)"),
                    subtitle_style,
                )
            )
            ...
            QMessageBox.critical(
                self, self.translate_text("Erreur"), self.translate_text("Erreur lors de l'export:") + f"\n{str(e)}"
            )

    def export_json(self):
        filepath, _ = QFileDialog.getSaveFileName(
            self, self.translate_text("Exporter l'historique en JSON"), "", "JSON (*.json)"
        )
