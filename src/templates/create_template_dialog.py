...
class CreateTemplateDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.memorize_check.setToolTip(
            self.parent_dialog.translate_text(
        ...
        self.set_as_default_check = AnimatedCheckBox(
            self.parent_dialog.translate_text("Définir comme template par défaut")
        )
        self.set_as_default_check.setToolTip(
            self.parent_dialog.translate_text(
                "Si coché, ce template s'applique silencieusement à chaque utilisation de cette opération, sans passer par le panel Templates."  # noqa: E501
            )
        )
    ...
    def create_compression_config(self):
        ...
        self.delete_originals_check = AnimatedCheckBox(
            self.parent_dialog.translate_text("Supprimer les fichiers originaux après compression")
        )
        self.delete_originals_check.setChecked(False)

        self.split_check = AnimatedCheckBox(
            self.parent_dialog.translate_text("Fractionner l'archive en plusieurs parties")
        )
    ...
    def create_pdf_protection_config(self):
        ...
        self.allow_modifications_check = AnimatedCheckBox(
            self.parent_dialog.translate_text("Autoriser les modifications")
        )
    ...
    def create_word_to_pdf_config(self):
        ...
        self.include_metadata_check = AnimatedCheckBox(self.parent_dialog.translate_text("Inclure les métadonnées"))
        self.include_metadata_check.setChecked(True)

        self.compress_images_check = AnimatedCheckBox(self.parent_dialog.translate_text("Compresser les images"))
    ...
    def create_template(self):
        ...
        if template_type == self.parent_dialog.translate_text("Conversion PDF→Word"):
            config_data = {"mode": self.mode_combo.currentText()}
        elif template_type == self.parent_dialog.translate_text("Conversion Word→PDF"):
            config_data = {
        ...
        if self.set_as_default_check.isChecked():
            ...
            for tid, tpl in self.template_manager.current_templates.items():
                if tpl["name"] == name and TemplateManager.normalize_type(
                    tpl["type"]
        ...
        if ach:
            try:
                ach.record_template_created(template_type)
            except Exception:
                pass
