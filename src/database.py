...
class DatabaseManager:
    ...
    def get_templates(self, template_type=None):
        ...
        if template_type:
            cursor.execute(
                "SELECT id, name, template_type, config_data, created_at, last_used FROM templates WHERE template_type = ? ORDER BY last_used DESC, created_at DESC",  # noqa: E501
                (template_type,),
            )
        else:
            cursor.execute(
                "SELECT id, name, template_type, config_data, created_at, last_used FROM templates ORDER BY last_used DESC, created_at DESC"  # noqa: E501
            )
    ...
    def update_template_config(self, template_id, config_data_json):
        ...
        conn = self._conn
        cursor = conn.cursor()
        cursor.execute("UPDATE templates SET config_data = ? WHERE id = ?", (config_data_json, template_id))
        conn.commit()
