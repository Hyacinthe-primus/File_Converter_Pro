...
class XPRingWidget(QWidget):
    ...
    def paintEvent(self, _event):
        ...
        if self.progress > 0:
            ...
            if self._complete:
                ...
                for i in range(12):
                    ...
                    p.drawArc(rect_outer, (90 - i * 30) * 16 + int(self._shimmer * 50), -seg_sweep)
        ...
        if self._complete:
            ...
            p.drawText(
                QRectF(cx - r_inner, cy - r_inner * 0.55, r_inner * 2, r_inner * 0.55),
            ...
            p.drawText(
                QRectF(cx - r_inner, cy - r_inner * 0.05, r_inner * 2, r_inner * 0.6),
            ...
            p.drawText(
                QRectF(cx - r_inner, cy + r_inner * 0.45, r_inner * 2, r_inner * 0.4),
                Qt.AlignHCenter | Qt.AlignTop,
                "TOTAL XP",
            )
        else:
            ...
            p.drawText(
                QRectF(cx - r_inner, cy - r_inner * 0.72, r_inner * 2, r_inner * 1.0),
                Qt.AlignHCenter | Qt.AlignVCenter,
                self._format_xp(self.total_xp),
            )
            ...
            p.drawText(
                QRectF(cx - r_inner, cy + r_inner * 0.12, r_inner * 2, r_inner * 0.45),
                Qt.AlignHCenter | Qt.AlignTop,
            ...
            p.setFont(QFont("Segoe UI", 7))
            p.drawText(
                QRectF(cx - r_inner, cy + r_inner * 0.48, r_inner * 2, r_inner * 0.35),
                Qt.AlignHCenter | Qt.AlignTop,
                "TOTAL XP",
            )
...
class StatBarWidget(QWidget):
    def __init__(self, label: str, value: str, fraction: float = 1.0, color: str = DS.BLUE, parent=None):
        super().__init__(parent)
        self.label = label
        self.value = value
    ...
    def paintEvent(self, _event):
        ...
        p.drawText(QRectF(0, 0, w * 0.7, h * 0.58), Qt.AlignVCenter | Qt.AlignLeft, self.label)

        p.setPen(self.color)
        p.setFont(QFont("Segoe UI", 9, QFont.Bold))
        p.drawText(QRectF(0, 0, w, h * 0.58), Qt.AlignVCenter | Qt.AlignRight, self.value)
...
class AchievementCard(QFrame):
    ...
    def paintEvent(self, _event):
        ...
        if secret:
            ...
            c_top = QColor(self._tier_color)
            c_top.setAlpha(45)
            c_mid = QColor(self._tier_glow)
            c_mid.setAlpha(30)
        ...
        if t > 0.001:
            if unlocked:
                ...
                core_bright = QColor(self._tier_color)
                core_bright.setAlpha(int(55 * t))
                core_mid = QColor(self._tier_color)
                core_mid.setAlpha(int(22 * t))
                core_edge = QColor(self._tier_color)
                core_edge.setAlpha(0)
                ...
                halo_c1 = QColor(hov_c)
                halo_c1.setAlpha(0)
                halo_c2 = QColor(hov_c)
                halo_c2.setAlpha(int(30 * t))
                halo_c3 = QColor(hov_c)
                halo_c3.setAlpha(int(55 * t))
                ...
                flood_c = QColor(cat_c)
                flood_c.setAlpha(int(38 * t))
                p.fillPath(flood_path, QBrush(flood_c))

                refract = QRadialGradient(w * 0.5, h * 0.5, max(w, h) * 0.72)
                edge_c = QColor(cat_c)
                edge_c.setAlpha(int(70 * t))
                mid_c = QColor(cat_c)
                mid_c.setAlpha(int(22 * t))
                core_c = QColor(cat_c)
                core_c.setAlpha(0)
                ...
                if t > 0.3:
                    ...
                    caustic_g = QLinearGradient(cx_pos - w * 0.35, 0, cx_pos + w * 0.35, h)
                ...
                if t < 0.95:
                    ...
                    if wl_alpha > 0:
                        ...
                        wl_fill = QColor(cat_c)
                        wl_fill.setAlpha(int(18 * t))
        ...
        stripe_color = (
            hov_c
            if (t > 0.01 and unlocked)
            else (cat_c if (t > 0.01 and not unlocked) else (QColor(DS.RED) if secret else self._tier_color))
        )
        ...
        sc0 = QColor(stripe_color)
        sc0.setAlpha(0)
        base_alpha = 0.55 if unlocked else (0.25 if not secret else 0.2)
        sc1 = QColor(stripe_color)
        sc1.setAlpha(int(220 * max(t, base_alpha)))
        sc2 = QColor(stripe_color)
        sc2.setAlpha(0)
        sg.setColorAt(0, sc0)
        sg.setColorAt(0.5, sc1)
        sg.setColorAt(1, sc2)
        ...
        if secret:
            badge_bg = QColor(DS.RED)
            badge_bg.setAlpha(int(18 + 28 * t))
        else:
            badge_bg = QColor(badge_col)
            badge_bg.setAlpha(int(25 + 45 * (t if unlocked else t * 0.6)))
        ...
        if secret:
            ...
            p.drawPixmap(int(ix + (ICON_SIZE - 38) / 2), int(iy + (ICON_SIZE - 38) / 2), self._pixmap)
            p.setOpacity(1.0)
        else:
            p.setOpacity(1.0 if unlocked else (0.3 + 0.35 * t))
            p.setFont(QFont("Segoe UI Emoji", 20))
            p.setPen(QColor(DS.TEXT_PRIMARY))
            p.drawText(QRectF(ix, iy, ICON_SIZE, ICON_SIZE), Qt.AlignCenter, "🏆" if unlocked else "❓")
            p.setOpacity(1.0)
        ...
        name_text = (
            self._tm.translate_text(_raw_name)
        ...
        if not secret and self.tier_data and self.category_data:
            ...
            tier_name = self._tm.translate_text(_tn) if isinstance(_tn, str) else _tn.get(self.language, "")
            _cn = self.category_data.get("name", "")
            cat_name = self._tm.translate_text(_cn) if isinstance(_cn, str) else _cn.get(self.language, "")
            ...
            sc3 = QColor(DS.TEXT_MUTED)
            sc3.setAlpha(int(100 + 80 * t))
            p.setPen(sc3)
            p.setFont(QFont("Segoe UI", 8))
        ...
        desc = (
            self._tm.translate_text(_raw_desc)
            if isinstance(_raw_desc, str)
            else (_raw_desc.get(self.language) or _raw_desc.get("fr", ""))
        )
        if secret:
            desc = (
                self._tm.translate_text("secret_achievement_desc")
                if hasattr(self, "_tm")
                else "Ce succès est encore secret. Continuez à explorer..."
            )
        ...
        p.setPen(dc)
        p.setFont(QFont("Segoe UI", 9))
        p.drawText(QRectF(tx, 53, tw, 32), Qt.AlignTop | Qt.AlignLeft | Qt.TextWordWrap, desc)
        ...
        if not unlocked and not secret:
            ...
            if frac > 0:
                bar_color = (
                    QColor("#F85149") if frac < 0.33 else QColor("#F0A030") if frac < 0.66 else QColor("#3FB950")
                )
                if t > 0:
                    bar_color = QColor(
            ...
            p.setPen(prog_c)
            p.setFont(QFont("Segoe UI", 7))
            p.drawText(QRectF(tx, bar_y - 13, bar_w, 12), Qt.AlignVCenter | Qt.AlignRight, f"{cur:.0f} / {req:.0f}")
        ...
        if unlocked:
            ...
            if ach.get("unlock_date"):
                try:
                    ...
                    pass
            gc3 = QColor(DS.GREEN)
            gc3.setAlpha(int(170 + 85 * t))
            p.setPen(gc3)
            p.drawText(QRectF(0, h - 20, w - 12, 14), Qt.AlignVCenter | Qt.AlignRight, badge_text)
        elif secret:
            sc4 = QColor(DS.RED)
            sc4.setAlpha(int(120 + 90 * t))
            p.setPen(sc4)
            p.drawText(QRectF(0, h - 20, w - 12, 14), Qt.AlignVCenter | Qt.AlignRight, "✦  SECRET  ✦")
...
class RankBadgeWidget(QWidget):
    ...
    def __init__(self, rank_name: str, rank_color: str, icon_path: str = "", parent=None):
        super().__init__(parent)
    ...
    def paintEvent(self, _event):
        ...
        if lv >= 2:
            ...
            if lv >= 4:
                arc_c2 = QColor.fromHsvF((self.rank_color.hsvHueF() + 0.33) % 1.0, 0.8, 1.0)
        ...
        if lv >= 3:
            ...
            def _draw_dot(phase_val: float, hue_shift: float, alpha_extra: int):
                ...
                if lv >= 4 and hue_shift != 0:
                    dot_c = QColor.fromHsvF((rc.hsvHueF() + hue_shift) % 1.0, 0.85, 1.0)
                else:
        ...
        p.setPen(rc)
        p.setFont(QFont("Segoe UI", 11, QFont.Bold))
        p.drawText(QRectF(text_x, 0, w - text_x - 8, h), Qt.AlignVCenter | Qt.AlignLeft, self.rank_name)
        p.end()
...
class AchievementsUI(TranslationMixin, QDialog):
    ...
    def _build_overview_tab(self) -> QWidget:
        ...
        max_xp = (
            self.achievement_system.get_max_xp()
            if hasattr(self.achievement_system, "get_max_xp")
            else sum(a.get("reward_xp", 0) for a in self.achievement_system.get_all_achievements().values())
        )
        ...
        self._category_tree.setHeaderLabels([self.T("Catégorie"), self.T("Progression")])
    ...
    def _build_list_tab(self) -> QWidget:
        ...
        self._skeleton_lbl.setStyleSheet(f"color: {DS.TEXT_MUTED}; font-size: 14px; padding: 40px;")
    ...
    def _load_filters(self):
        ...
        for cat_id, cat_data in categories.items():
            _n = cat_data["name"]
            name = self.T(_n) if isinstance(_n, str) else _n.get(self.current_language, _n.get("fr", ""))
    ...
    def load_overview(self):
        ...
        for cat_id, cstats in ach_sys.get_category_stats().items():
            ...
            if cat_data:
                ...
                item.setText(0, name)
                item.setText(1, f"{cstats['unlocked']}/{cstats['total']}  ({cstats['percentage']:.0f}%)")
    ...
    def load_achievements_list(self):
        ...
        self._cards_layout.addItem(
            QSpacerItem(0, 0, QSizePolicy.Minimum, QSizePolicy.Expanding), row + 1, 0, 1, max_cols
        )
    ...
    def _filter_achievements_list(self, achievements: dict) -> dict:
        ...
        for aid, ach in achievements.items():
            ...
            if search:
                _rn = ach["name"]
                name = (
                    self.T(_rn) if isinstance(_rn, str) else _rn.get(self.current_language, _rn.get("fr", ""))
                ).lower()
                _rd = ach["description"]
                desc = (
                    self.T(_rd) if isinstance(_rd, str) else _rd.get(self.current_language, _rd.get("fr", ""))
                ).lower()
